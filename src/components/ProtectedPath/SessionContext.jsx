...
export const SessionProvider = ({ children }) => {
  ...
  // NEW: details state to store fetched details from API
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      try {
        const res = await axiosInstance.get("/details.php");
        setDetails(res.data);
        console.log("Fetched details:", res.data); // debugging
      } catch (error) {
  ...
  // Expose details as well in context value
  return (
    <SessionContext.Provider
      value={{ session, setSession, updateSession, logout, details }}
    >
      {children}
    </SessionContext.Provider>
  );
