      ...
      <div className="w-full h-full flex gap-10 p-8 flex-col">
        ...
        <div className="w-full grid grid-cols-4 gap-4">
          {features.map((feature, index) => {
            return (
              <div
                className="border-solid border-2 cursor-pointer p-2 bg-gray-100 relative"
                key={feature}
                onClick={() => setPopup(feature)}
              >
                <img
                  src={`admin/feature_${index + 1}.png`}
      ...
      {popup !== "none" && (
        <div className="fixed top-0 left-0 p-14 w-full h-full flex justify-center items-center bg-gray-700 bg-opacity-70">
          ...
          <button
            ...
            className="absolute top-4 right-4 bg-white px-2 rounded-lg"
          >
            Close
          </button>
        </div>
