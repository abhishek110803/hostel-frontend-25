...
export default function EditDatabase() {
    ...
    const deleteRow = async (row) => {
        try {
            await toast.promise(
                axiosInstance.post('/delete_row.php', {
                    table: tables,
                    rowData: row
                }, {
    ...
    const handleTableChange = async (e) => {
        ...
        try {
            ...
            setNewRowData(Object.fromEntries(Object.keys(res.data.data[0] || {}).map(k => [k, ''])));
    ...
    const submitEdit = async () => {
        ...
        try {
            const promise = axiosInstance.post('/edit_row.php', {
                table: tables,
    ...
    return (
        <div className="w-full min-h-full p-6 border border-gray-300 bg-gray-100 flex flex-col gap-2">
            ...
            <div className="mb-4">
                ...
                <button className="bg-green-600 text-white px-4 py-1 rounded hover:bg-green-700 ml-2" onClick={() => setAddModalOpen(true)}>Add New Entry</button>
                                    ...
                                    {tableData.map((row, rowIdx) => (
                                        <tr key={rowIdx}>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 border">
                                                <button className="text-blue-600 hover:underline mr-2" onClick={() => openEditModal(row)}>Edit</button>
                                                <button className="text-red-600 hover:underline" onClick={() => deleteRow(row)}>Delete</button>
                                            </td>
                                ...
                                <div className="mt-6 flex justify-end gap-4">
                                    ...
                                    <button
                                        ...
                                        onClick={async () => {
                                            ...
                                            try {
                                                const promise = axiosInstance.post('/insert_row.php', {
                                                    table: tables,
                                                    row_data: newRowData
                                                }, {
                                                    headers: { 'Content-Type': 'application/json' }
                                                ...
                                                await toast.promise(promise, {
                                                    ...
                                                    success: (res) => {
                                                        if (res.data.success) {
                                                            setOriginalTableData(prev => [...prev, newRowData]);
